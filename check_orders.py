...
MAILBOX = os.environ.get("MAILBOX", "INBOX")              # e.g. "[Gmail]/All Mail" if you prefer
...
def fetch_sheet_rows(url):
    print(f"[info] Fetching sheet CSV: {url}")
    try:
        req = Request(url, headers={"User-Agent":"Mozilla/5.0"})
        with urlopen(req, timeout=30) as r:
            content = r.read().decode("utf-8", errors="ignore")
    except HTTPError as e:
        die(f"[error] Failed to fetch CSV (HTTP {e.code}). Is the sheet shared/public or published to web?")
    except URLError as e:
        die(f"[error] Failed to fetch CSV (network error): {e.reason}")
    rows = list(csv.reader(content.splitlines()))
    if not rows:
        print("[warn] CSV contains no rows at all.")
        return []
    header = [h.strip().lower() for h in rows[0]]
    data = []
    for r in rows[1:]:
        row = {header[i]: (r[i].strip() if i < len(header) and i < len(r) else "") for i in range(len(header))}
        # consider a row only if it has at least an order number
        if any(v for v in row.values()):
            data.append(row)
    print(f"[info] Parsed {len(data)} data row(s) from sheet.")
    return data

def imap_connect():
    if not (IMAP_EMAIL and IMAP_APP_PASSWORD):
        die("[error] Missing IMAP_EMAIL/IMAP_APP_PASSWORD environment variables (GitHub Secrets).")
    try:
        imap = imaplib.IMAP4_SSL(IMAP_SERVER)
        imap.login(IMAP_EMAIL, IMAP_APP_PASSWORD)
        imap.select(MAILBOX)
        print(f"[info] IMAP login OK. Selected mailbox: {MAILBOX}")
        return imap
    except imaplib.IMAP4.error as e:
        die(f"[error] IMAP login failed: {e}. "
            "Enable IMAP in Gmail settings, turn on 2-Step Verification, create a fresh App Password, "
            "and consider approving any recent security alert in your Google Account.")
...
def search_latest_for_order(imap, order_number, site_hint=""):
    ...
    if site_hint:
        crit = f'(TEXT "{order_number}" TEXT "{site_hint}")'
...
def main():
    if not SHEET_CSV_URL:
        die("[error] Missing SHEET_CSV_URL secret.")
    rows = fetch_sheet_rows(SHEET_CSV_URL)

    # Always write a CSV so downstream steps can proceed
    os.makedirs("data", exist_ok=True)
    out_path = "data/status.csv"
    with open(out_path, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["order_number","site","status","tracking","carrier","email_date","subject"])

        if not rows:
            print("[info] No sheet data rows yet; wrote empty status.csv.")
        ...
        try:
            for r in rows:
                ...
                if not msg:
                    w.writerow([order, site, "Pending", "", "", "", ""])
                    continue
                subject = dec(msg.get("Subject") or "")
                date_hdr = dec(msg.get("Date") or "")
                body = get_body(msg)
